...
const GLYPH_FONT_3ROW = {
  // W is 5 cols wide (vs. 3 for the rest) — at 3 cols the only shape
  // available is two strokes meeting at a single bottom point, which reads
  // as V, not W. 5 cols gives it two valleys + a middle peak so it's
...
function initGlyphStrip(){
  ...
  const ROWS = 3;
  // Derived from the actual glyph data rather than hardcoded, so column
  // count always matches what buildWordPattern produces (currently 29:
  // W=5 + E/L/C/O/M/E=3 each (18) + 6 single-col gaps = 29).
  const COLS = buildWordPattern('WELCOME')[0].length;
  ...
  function playWelcome(){
    ...
    const cols = pattern[0].length; // matches COLS — see buildWordPattern
